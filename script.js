...
function playJumpscareSound(){
    try{
        ...
        // grito sintetizado: ruído + osc descendente distorcido
        ...
        noiseGain.gain.exponentialRampToValueAtTime(0.0001,ctx.currentTime+0.8);
        ...
        oscGain.gain.exponentialRampToValueAtTime(0.0001,ctx.currentTime+0.8);
        osc.connect(oscGain);
...
function tryLogin(){
    ...
    setTimeout(()=>{loginStatus.innerText="Descriptografando sessão...";},600);
...
/* Ruídos de interferência aleatórios */
...
/* Fake alerta estilo antivírus/malware */
const scanStatus=document.getElementById("scanStatus");
const malwareToast=document.getElementById("malwareToast");

const fakeThreats=[
    "trojan.fake.exe",
    "worm.simulado.dll",
    "script.suspeito.js",
    "adware.brincadeira.tmp"
];

function scheduleMalwareAlerts(){
    const delay=25000+Math.random()*25000;
    setTimeout(()=>{
        const threat=fakeThreats[Math.floor(Math.random()*fakeThreats.length)];
        playGlitchNoise();
        malwareToast.innerHTML=`⚠ Ameaça detectada: ${threat} — Neutralizada ✔`;
        malwareToast.style.display="block";
        setTimeout(()=>{malwareToast.style.display="none";},3200);
        terminalLine(`⚠ [ANTIVIRUS] ${threat} detectado — quarentena aplicada.`,"#ff003c");
        scheduleMalwareAlerts();
    },delay);
}

function runFakeScan(){
    const messages=["🔎 Scanning...","🛡 Verificando integridade...","🔎 Escaneando memória...","🛡 0 ameaças ativas"];
    let i=0;
    setInterval(()=>{
        scanStatus.innerText=messages[i%messages.length];
        i++;
    },4000);
}

/* Título assustador ao trocar de aba */
const originalTitle=document.title;

document.addEventListener("visibilitychange",()=>{
    document.title=document.hidden?"👀 Volte já...":originalTitle;
});
...
/* ======================================================
   BOTÃO "NÃO CLIQUE AQUI" — jumpscare
====================================================== */
...
extractInput.onchange=()=>{
    if(extractInput.files.length){
        extractFileName.innerText="📄 "+extractInput.files[0].name;
    }
};
...
watermarkInput.onchange=()=>{
    if(watermarkInput.files.length){
        watermarkFileName.innerText="📄 "+watermarkInput.files[0].name;
    }
};
...
async function extractPages(){
    ...
    if(!rangeStr){
        toast("Informe as páginas (ex: 1-3,5).");
        return;
    }
    ...
    if(!indices.length){
        ...
        terminalLine("✖ Nenhuma página válida encontrada no intervalo informado.","#ff003c");
        return;
    }
...
async function addWatermark(){
    ...
    if(!text){
        toast("Digite o texto da marca d'água.");
        return;
    }
    ...
    shell("💧 Aplicando marca d'água em "+pages.length+" página(s)...");
    ...
    shell("💾 Gerando PDF com marca d'água...");
...
/* ---- Verificador de senha vazada (k-anonimato, estilo Have I Been Pwned) ---- */
...
async function checkPasswordLeak(){
    ...
    if(!senha){
        toast("Digite uma senha para verificar.");
        return;
    }
    ...
    try{
        ...
        if(count>0){
            ...
            playDenied();
            terminalLine("⚠ [LEAK CHECK] Senha encontrada em vazamentos: "+count,"#ff003c");
        }else{
            resultDiv.innerHTML="✔ Boas notícias — essa senha não apareceu em vazamentos conhecidos.";
            resultDiv.style.color="#00ff80";
            playGranted();
            terminalLine("✔ [LEAK CHECK] Nenhum vazamento encontrado para essa senha.","#55ff55");
        }
...
/* ======================================================
   MINI-GAME: "FIREWALL RUNNER" (estilo dinossauro do Chrome)
   -- CORRIGIDO: o jogo agora inicia corretamente no primeiro
      clique/toque/espaço, pois "over" começa true (estado
      "aguardando início"), e jump() ao ver over=true chama
      resetGame(), que é quem de fato inicia o loop do jogo.
====================================================== */
...
function setupGame(){
    ...
    let score=0;
    let frame=0;
    let over=true; // começa "parado", esperando o primeiro toque/clique/espaço
    let loopId=null;
    ...
    function update(){
        ...
        if(frame%Math.max(40,Math.floor(90-speed*4))===0){
            spawnObstacle();
        }
    ...
    function endGame(){
        over=true;
        playGameOver();
        gameOverMsg.innerText="💀 GAME OVER — score "+Math.floor(score)+" — toque/clique ou ESPAÇO para tentar de novo";
