...
/* Configura o worker do pdf.js (necessário pra renderizar páginas) */
...
async function ipLookup(){
    ...
    try{
        ...
        if(data.error){
            resultDiv.innerText="✖ IP inválido ou não encontrado.";
            resultDiv.style.color="#ff003c";
            return;
        }
        ...
        resultDiv.innerHTML=`
            ...
            <b>País:</b> ${data.country_name||"?"}<br>
            <b>Provedor (ISP):</b> ${data.org||"?"}<br>
            <b>Fuso horário:</b> ${data.timezone||"?"}<br>
        ...
        resultDiv.innerText="✖ Não foi possível consultar agora.";
        resultDiv.style.color="#ff003c";
    }
}
